...
_refR = re.compile('</ref>', re.IGNORECASE)
_referencesR = re.compile('<references.*?/>', re.IGNORECASE)


def _match_xml_page_text(text):
    """Match page text."""
    text = textlib.removeDisabledParts(text)
    return _refR.search(text) and not _referencesR.search(text)
