...
def is_simple_locale_with_region(locale):
    ...
    if '-' in lang:
        return False
    # Only allow qualifiers that look like a country code, with any suffix
...
def generate_locale(lang, region=True, encoding='utf8'):
    ...
    if region is True:
        ...
        # Get the region from the first locale
...
def temp_text_file_do(text, command, **kwargs):
    """
...
def win32_set_locale(locale_code):
    ...
    temp_text_file_do(locale_update_xml, command, suffix='.xml')
...
def execute(command, data_in=None, timeout=0, error=None):
    ...
    if pywikibot.config.userinterface_lang:
        ...
        locale_prefix = str(pywikibot.config.userinterface_lang + '_')
