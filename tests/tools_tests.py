...
class StopList(set):
    ...
    def __contains__(self, item):
        ...
        if item in self.stop_list:
            ...
            return super(StopList, self).__contains__(item)
...
class TestFilterUnique(TestCase):
    ...
    def _test_dedup_int(self, deduped, deduper, key=None):
        ...
        if key in (hash, passthrough):
            if isinstance(deduped, tools.OrderedDict):
                ...
                self.assertEqual(list(deduped.keys()), [1, 2, 3, 4])
            else:
    ...
    def test_obj_hash(self):
        """Test filter_unique with objects using hash."""
        deduped = set()
    ...
    def test_obj_id(self):
        """Test filter_unique with objects, failing when using id as key."""
        deduped = set()
