...
class OpenArchiveTestCase(TestCase):
    ...
    def test_open_archive_without_bz2(self):
        ...
        old_bz2 = tools.bz2
        old_bz2_import_error = getattr(tools, 'bz2_import_error', None)
        try:
            tools.bz2 = False
            tools.bz2_import_error = ImportError
            self.assertRaises(ImportError, self._get_content, self.base_file + '.bz2')
        finally:
            tools.bz2 = old_bz2
            tools.bz2_import_error = old_bz2_import_error
            if tools.bz2_import_error is None:
                del tools.bz2_import_error
