    ...
    def _get_siteinfo(self, prop, expiry):
        ...
        try:
            request = self._site._request(
                ...
                parameters={'action': 'query', 'meta': 'siteinfo', 'siprop': props})
...
"[[%(newtitle)s]] file extension does not match content of [[%(oldtitle)s]]",
    ...
    def undelete_page(self, page, reason, revisions=None):
        ...
        """
        parameters = {'action': 'undelete', 'title': page, 'reason': reason,
                      'token': self.tokens['undelete']}
        if revisions is not None:
            parameters['timestamps'] = revisions
        self.lock_page(page)
        req = self._simple_request(**parameters)
        try:
            req.submit()
            ...
            with open(source_filename, 'rb') as f:
                ...
                if chunked_upload:
                    ...
                    filetype = (mimetypes.guess_type(source_filename)[0] or
                                'application/octet-stream')
                    final_request.mime_params['file'] = (
                        file_contents, filetype.split('/'),
                        {'filename': mime_filename})
        else:
            # upload by URL
