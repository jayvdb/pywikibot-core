...
def filter_unique(iterable, container=None, key=None, add=None):
    ...
    container and use L{hash} or a L{weakref} as the key.
    ...
    used automatically.  Any other method may be provided explicitly.
    ...
    if not add:
        if hasattr(container, 'add'):
            ...
            add = lambda x: container_add(key(x))
        else:
            container_setitem = container.__setitem__
            add = lambda x: container_setitem(key(x), True)

    for item in iterable:
        if key(item) not in container:
            add(item)
            yield item
