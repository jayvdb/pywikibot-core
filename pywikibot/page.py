...
class WikibasePage(BasePage):
    ...
    def get(self, force=False, *args, **kwargs):
        ...
        nested_keys = set(
            key for key, value in self._content.items()
            if isinstance(value, dict) or isinstance(value, list))
        assert nested_keys <= self._content_keys, \
            'Unknown keys: {0}'.format(nested_keys - self._content_keys)

        content_keys = set(self._content) - self._metadata_keys
        assert content_keys <= self._content_keys, \
            'Unknown keys: {0}'.format(content_keys - self._content_keys)
