...
class QueryGenerator(object):
    ...
    @staticmethod
    def querydata_update(orig_dict, new_dict):
        ...
        for key, val in new_dict.items():
            if isinstance(val, Mapping):
                orig_dict[key] = QueryGenerator.querydata_update(
    ...
    def __iter__(self):
            ...
            if self.resultkey in self.data["query"]:
                ...
                if isinstance(resultdata, dict):
                    ...
                    if "pageids" in self.data["query"]:
                        query_keys = self.data["query"]["pageids"]
                    else:
                        # FIXME: these keys may be pageids as a string, which
                        # causes an ascii ordering, not a numerical order.
                        query_keys = sorted(resultdata.keys())
                ...
                for pageid in query_buffer:
                    if pageid not in resultdata:
                        yield self.result(query_buffer.pop(pageid))

                for pageid, item in resultdata.items():
                    # Insert or update page in query_buffer.
                    querydata = query_buffer.get(pageid, {})
                    querydata = QueryGenerator.querydata_update(querydata, item)
                    query_buffer[pageid] = querydata
        ...
        # Flush buffer, if any, when completing the iteration.
        for item in query_buffer.values():
