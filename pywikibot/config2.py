...
for arg in sys.argv[1:]:
    if arg.startswith(str('-verbose')) or arg == str('-v'):
        output("The base directory is %s" % base_dir)
        break
family_files = {}
...
def _win32_extension_command(extension):
    ...
    try:
        ...
        warning('Unable to find editor for files *.' + extension)
        error(e)
...
for _filename in _fns:
    ...
    if os.path.exists(_filename):
        ...
        if sys.platform == 'win32' or _fileuid in [os.getuid(), 0]:
            ...
            warning("WARNING: Skipped '%(fn)s': owned by someone else."
                    % {'fn': _filename})
...
if transliteration_target == 'not set':
    if sys.platform == 'win32':
        ...
        warning(
            ...
            '\nPlease see '
            'https://www.mediawiki.org/wiki/Manual:Pywikibot/Windows')
    else:
        transliteration_target = None
...
if sys.platform == 'win32' and editor:
    ...
    if set(editor) & set('\a\b\f\n\r\t\v'):
        warning('The editor path contains probably invalid escaped characters.'
                ' Make sure to use a raw-string (r"..." or r\'...\'), '
